...
export default class ScatterWallet{
    ...
    // connect to scatter
    connect(callback){
        let {dappName,initTimeout} = this.config
        let connectionOptions = {initTimeout:initTimeout}
        ScatterJS.scatter.connect(dappName,connectionOptions).then(connected=>{
    ...
    transfer(from,to,count,memo,callback){
        ...
        if(callback){
            this.eos.transfer(from, to, parseFloat(count).toFixed(4) + ' EOS',memo,this.transactionOptions).then(trx=>{
                callback(null,trx)
            }).catch(error=>{
                callback(error,null)
            })
